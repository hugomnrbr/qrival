...
-- ---------------------------------------------------------------
-- 3) INVENTÁRIO + REAÇÕES DA PARTIDA
-- ---------------------------------------------------------------
...
create policy "match reactions read authenticated" on public.match_reactions for select to authenticated using(true);
drop policy if exists "match reactions insert own" on public.match_reactions;
create policy "match reactions insert own" on public.match_reactions for insert to authenticated with check(sender_id=auth.uid());
drop policy if exists "match reactions delete own" on public.match_reactions;
create policy "match reactions delete own" on public.match_reactions for delete to authenticated using(sender_id=auth.uid());
do $$ begin
...
set search_path=public
as $$
declare r public.match_reactions;
begin
  if auth.uid() is null then raise exception 'Não autenticado'; end if;
  if coalesce(trim(p_emoji),'')='' then raise exception 'Emoji inválido'; end if;
  if not exists(select 1 from public.matches m where m.id=p_match_id and auth.uid()=any(m.player_ids) and m.status<>'finished') then
